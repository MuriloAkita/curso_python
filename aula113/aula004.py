...
@Slot()
def slotFecharJanela():
    window.close()

@Slot()
def slotOnCheck(checked):
    print('Está marcado ?', checked)

@Slot()    
def slotVerificarCheck(action):
    def inner():
        slotOnCheck(action.isChecked())
    return inner

@Slot()
def slotExibirVersao(status_bar):
...
fecharAction.triggered.connect(slotFecharJanela)

testeAction = arquivoMenu.addAction('teste')
testeAction.setCheckable(True)
testeAction.toggled.connect(slotOnCheck) # Quando marcar...
testeAction.hovered.connect(slotVerificarCheck(testeAction)) # Quando passar o mouse em cima...

botao.clicked.connect(slotVerificarCheck(testeAction))

ajudaMenu = menu.addMenu('Ajuda')
versaoAction = ajudaMenu.addAction('Versão')
versaoAction.triggered.connect(lambda: slotExibirVersao(status_bar))
